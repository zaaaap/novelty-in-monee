...
calc_pressure <- function(windata, inddata){
  ...
  for(j in 0:9) {
    ...
    for (i in 0:999) {
      ...
      mean_pucks <- mean(currentdd$V3)
      mean_offspring <- mean(currentdd$N)
      #create contingency table and calc pressure
      cont <- matrix(c(nrow(currentdd[which(currentdd$N <= mean_offspring & currentdd$V3 <= mean_pucks)]),
                       nrow(currentdd[which(currentdd$N > mean_offspring & currentdd$V3 <= mean_pucks)]),
                       nrow(currentdd[which(currentdd$N <= mean_offspring & currentdd$V3 > mean_pucks)]),
                       nrow(currentdd[which(currentdd$N > mean_offspring & currentdd$V3 > mean_pucks)])),
                     nrow = 2, ncol = 2)
...
plotname = "puck_pressure_arch.pdf"
