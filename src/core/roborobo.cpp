...
void loadSimSensorCSV(std::string filepath) {
// Load CSV of sample sensor input values for comparing behavior on different scenarios.
	...
	while (getline(file, line)) {
		...
		simSensorInput.push_back(result);
	}
}
...
int gChangeEnvOn;
int gKNNeighbours;
int gPucksCollected = 0;
int gPucksCollected1 = 0;
int gPucksCollected2 = 0;
int gMaxArchiveSize;
bool gWithSync;
int gFitChoice = 0;
int gNovChoice = 0;
int gSelectionFunction;
int gMaxRepeatSelection;
int gSampleSize;
std::vector<int> gWinners;
std::vector<int> poolSizes;
int addedToArchCount = 0;
...
std::vector< std::vector<double> > simSensorInput;


// std::string gStartTime = getCurrentTimeAsReadableString();
std::string gStartTime = std::to_string(time(0));
...
// *********************************** NOV ***********************************
	...
	std::string filename = gProperties.getProperty("behaviourSampleFile");
	if (!(filename == "stats/behaviour-sampling/sample_sensors-20.csv" || filename == "stats/behaviour-sampling/sample_sensors-50.csv" || filename == "stats/behaviour-sampling/sample_sensors-100.csv")) {
		std::cerr << "[CORRUPT] behaviourSampleFile is missing.\n";
		returnValue = false;
	}

	loadSimSensorCSV(filename); // and save to simSensorInput

	if (gProperties.hasProperty("gFitnessMode")) {
		convertFromString<int>(gFitnessMode, gProperties.getProperty("gFitnessMode"), std::dec);
		
		if((gFitnessMode < 0)) {
			std::cerr << "[CORRUPT] gFitnessMode value not in range.\n";
			returnValue = false;
		}
	} else {
		std::cerr << "[MISSING] gFitnessMode value is missing.\n";
		returnValue = false;
	}

	if (gProperties.hasProperty("gSelectionFunction")) {
		convertFromString<int>(gSelectionFunction, gProperties.getProperty("gSelectionFunction"), std::dec);
		
		if((gFitnessMode < 0)) {
			std::cerr << "[CORRUPT] gSelectionFunction value not in range.\n";
			returnValue = false;
		}
	} else {
		std::cerr << "[MISSING] gSelectionFunction value is missing.\n";
		returnValue = false;
	}

	double temp = 0.0;
	if (gProperties.hasProperty("gChangeEnvOn")) {
		convertFromString<double>(temp, gProperties.getProperty("gChangeEnvOn"), std::dec);
		
		if(!(temp >= 0  && temp <= 1)) {
			std::cerr << "[CORRUPT] gChangeEnvOn value not in range.\n";
			returnValue = false;
		}
	} else {
		std::cerr << "[MISSING] gChangeEnvOn value is missing.\n";
		returnValue = false;
	}
	gChangeEnvOn = gMaxIt * temp; // Convert to iteration # of change

	if (gProperties.hasProperty("gKNNeighbours")) {
		convertFromString<int>(gKNNeighbours, gProperties.getProperty("gKNNeighbours"), std::dec);
	...
	if (gProperties.hasProperty("gMaxArchiveSize")) {
		convertFromString<int>(gMaxArchiveSize, gProperties.getProperty("gMaxArchiveSize"), std::dec);
		
		if(!(gMaxArchiveSize >= 0)) {
			std::cerr << "[CORRUPT] gMaxArchiveSize value not in range.\n";
			returnValue = false;
		}
	} else {
		std::cerr << "[MISSING] gMaxArchiveSize value is missing.\n";
		returnValue = false;
	}

	if (gProperties.hasProperty("gMaxRepeatSelection")) {
		convertFromString<int>(gMaxRepeatSelection, gProperties.getProperty("gMaxRepeatSelection"), std::dec);
		
		if(!(gMaxRepeatSelection > 0)) {
			std::cerr << "[CORRUPT] gMaxRepeatSelection value not in range.\n";
			returnValue = false;
		}
	} else {
		std::cerr << "[MISSING] gMaxRepeatSelection value is missing.\n";
		returnValue = false;
	}

	if (gProperties.hasProperty("gSampleSize")) {
		convertFromString<int>(gSampleSize, gProperties.getProperty("gSampleSize"), std::dec);
		
		if(!(gSampleSize > 0)) {
			std::cerr << "[CORRUPT] gSampleSize value not in range.\n";
			returnValue = false;
		}
	} else {
		std::cerr << "[MISSING] gSampleSize value is missing.\n";
		returnValue = false;
	}

	gWithSync = false;
	s = gProperties.getProperty("gWithSync");
	if (s == "true" || s == "True" || s == "TRUE")
		...
		std::cerr << "[WARNING] gWithSync is missing (default is \"false\").\n";
	}	
